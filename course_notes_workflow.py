...
SEEC_DEFAULT = "通识选修"
BELONGING_LINE_PATTERN = re.compile(r"^Belonging:\s*\[\[([^\]]+)\]\]\s*$", re.MULTILINE)
SLIDES_LINE_PATTERN = re.compile(r"^Slides:\s*(.+)$", re.MULTILINE)
COURSE_SECTION_HEADING = "## 课程讲义"
...
def _derive_belonging(snapshot: dict[str, Any]) -> str:
    # 0. 环境变量显式指定优先
    env_raw = os.getenv("COURSE_BELONGING", "").strip()
    if env_raw:
        return _sanitize_filename(env_raw)

    # 1. 优先匹配课程代码模式如 "2025-spring"
    preview = snapshot.get("page", {}).get("text_preview", "")
    match = re.search(r"(\d{4}-[^\s]+)", preview)
    if match:
        return _sanitize_filename(match.group(1))

    # 2. 从页面标题提取： "课程： 计算机视觉" → "计算机视觉"
    title = snapshot.get("page", {}).get("title", "").strip()
    if title:
        cleaned = re.sub(r"^(课程|Course)\s*[：:]\s*", "", title).strip()
        if cleaned and len(cleaned) <= 50:
            return _sanitize_filename(cleaned)

    # 3. 在正文中匹配 "课程名(年份)" 如 "计算机视觉(2026)"
    match = re.search(r"(\S+)\(\d{4}\)", preview)
    if match:
        return _sanitize_filename(match.group(1))
...
- 小节使用 `###`
- 以中文为主，保留必要英文术语
- 正文不要主动创建新的 `[[...]]` 概念双链；提到术语时直接写普通文本
- 直接输出 Markdown 正文，不要用 ```markdown 或其他代码块围栏包裹整篇答案
...
{trimmed_pdf_text}
""".strip()
    return _strip_outer_markdown_fence(_invoke_llm(prompt))
...
def _extract_note_belonging(note_content: str) -> str:
    match = BELONGING_LINE_PATTERN.search(note_content)
    if not match:
        return ""
    return match.group(1).strip()


def _is_material_note(note_content: str) -> bool:
    return SLIDES_LINE_PATTERN.search(note_content) is not None


def _iter_course_notes(notes_dir: Path, belonging: str) -> list[Path]:
    matched: list[Path] = []
    for md_file in sorted(notes_dir.glob("*.md")):
        if md_file.stem == belonging:
            continue
        content = md_file.read_text(encoding="utf-8")
        if _extract_note_belonging(content) == belonging:
            matched.append(md_file)
    return matched


def _extract_course_section_entries(note_content: str) -> dict[str, str]:
    entries: dict[str, str] = {}
    section_match = COURSE_SECTION_PATTERN.search(note_content)
    ...
    for raw_line in section_match.group("body").splitlines():
        ...
        if not match:
            continue
        entries[match.group(1).strip()] = line
    return entries


def _ensure_belonging_note(
    belonging: str,
    notes_dir: Path,
    processed: list[dict[str, Any]],
) -> Path | None:
    """Create/update the course-level note while preserving non-managed content."""
    note_path = notes_dir / f"{belonging}.md"
    has_existing = note_path.exists()
    existing_content = note_path.read_text(encoding="utf-8") if has_existing else ""
    now = datetime.now().strftime("%Y-%m-%d %H:%M")
    lecture_entries = _extract_course_section_entries(existing_content)

    for item in processed:
        note_path_str = str(item.get("note_path", "")).strip()
        status = str(item.get("status", "")).strip()
        if not note_path_str or status not in {"processed", "skipped-existing-note"}:
            continue
        lecture_entries[Path(note_path_str).stem] = f"- [[{Path(note_path_str).stem}]]"

    for md_file in _iter_course_notes(notes_dir, belonging):
        content = md_file.read_text(encoding="utf-8")
        if not _is_material_note(content):
            continue
        lecture_entries.setdefault(md_file.stem, f"- [[{md_file.stem}]]")

    if not lecture_entries:
        print(f"  -> skip course note: no lecture notes found for {belonging}")
        return None
    ...
    managed_section = "\n".join(
        [
            ...
            "",
            *[lecture_entries[key] for key in sorted(lecture_entries)],
        ]
    ).rstrip()

    if has_existing:
        content = existing_content
        if re.search(r"^>\s*更新：.*$", content, flags=re.MULTILINE):
            content = re.sub(r"^>\s*更新：.*$", f"> 更新：{now}", content, count=1, flags=re.MULTILINE)
        elif re.search(r"^#\s+.*$", content, flags=re.MULTILINE):
            content = re.sub(r"(^#\s+.*$)", r"\1\n\n" + f"> 更新：{now}", content, count=1, flags=re.MULTILINE)
        else:
            content = f"# {belonging}\n\n> 更新：{now}\n\n" + content.lstrip()

        if COURSE_SECTION_PATTERN.search(content):
            content = COURSE_SECTION_PATTERN.sub(managed_section + "\n\n", content, count=1).rstrip() + "\n"
        else:
    ...
    action = "已更新" if has_existing else "已创建"
    print(f"  -> course note {action}: {note_path}")
    return note_path
...
def process_selected_node(state: WorkflowState) -> WorkflowState:
    ...
    for item in state.get("opened_materials", []):
        ...
        try:
            ...
            note_content = _strip_outer_markdown_fence(note_content)
    ...
    # 创建/更新课程级笔记（Belonging 目标笔记）
    _ensure_belonging_note(belonging, notes_dir, processed)
